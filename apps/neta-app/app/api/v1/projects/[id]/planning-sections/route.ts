...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) { try { return apiV1Success(listOwnerPlanning(await requireApiV1Role(new Headers(request.headers), ["freelancer"], deviceScopesForRoute(["projects", "[id]", "planning-sections"], request.method)), request, (await params).id)); } catch (error) { return apiV1Error(error); } }
