...
export async function GET(request: Request) { try { return apiV1Success(getOwnerDashboardOverview(await requireApiV1Role(new Headers(request.headers), ["freelancer"]), request)); } catch (error) { return apiV1Error(error); } }
