...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) { try { return apiV1Success(getOwnerClient(await requireApiV1Role(new Headers(request.headers), ["freelancer"], deviceScopesForRoute(["clients", "[id]"], request.method)), request, (await params).id)); } catch (error) { return apiV1Error(error); } }
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) { try { const context = await requireApiV1Role(new Headers(request.headers), ["freelancer"], deviceScopesForRoute(["clients", "[id]"], request.method)); return apiV1Success(updateOwnerClient(context, request, (await params).id, await parseApiV1Json(request, z.unknown()))); } catch (error) { return apiV1Error(error); } }
